...
func TestLSSSCompleteFlow(t *testing.T) {
	...
	msp, err := BooleanToMSP(policy, false)
	...
	shareResult, err := LSSSShare(msp, secret, p)
	require.NoError(t, err, "生成份额时出错")
	require.NotNil(t, shareResult, "份额结果不应为nil")

	// 打印生成的份额
	t.Log("生成的份额 λi:")
	for i, lambda := range shareResult.lambda {
	...
	for i := 0; i < 2; i++ {
		if lambda, exists := shareResult.lambda[i]; exists {
	...
	// 4. 测试 LSSSRecon：重构秘密
	reconstructed, err := LSSSRecon(msp, shares, p)
