...
func Proof(x *big.Int, u, y1, v, y2 *bn256.G1) (*prfs, error) {
	//生成承诺
	r, err := rand.Int(rand.Reader, bn256.Order)
	if err != nil {
		return nil, err
	}
	a := new(bn256.G1).ScalarMult(u, r)
...
func Verify(pi *prfs, u, v, y1, y2 *bn256.G1) bool {
	ut := new(bn256.G1).ScalarMult(u, pi.T)
	vt := new(bn256.G1).ScalarMult(v, pi.T)
	cy1 := new(bn256.G1).ScalarMult(y1, pi.C)
	cy2 := new(bn256.G1).ScalarMult(y2, pi.C)
	a := new(bn256.G1).Add(ut, cy1)
